...
PeerList.prototype.getHost = function getHost(uniq) {
  ...
  for (i = 0; i < 100; i++) {
    ...
    if (uniq && this.get(addr.hostname))
      continue;
...
HostList.prototype.freshBucket = function freshBucket(entry) {
  ...
  index = murmur3(buf, 0xfba4c795) % this.fresh.length;
...
HostList.prototype.usedBucket = function usedBucket(entry) {
  ...
  var hash = murmur3(buf, 0xfba4c795);
...
HostList.prototype.add = function add(addr, src) {
  ...
  if (entry) {
    ...
    if (entry.refCount === this.maxMultiple)
      return false;

    assert(entry.refCount < this.maxMultiple);
