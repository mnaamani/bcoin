...
PeerList.prototype.getHost = function getHost(uniq) {
  var now = util.now();
  var i, entry;
  ...
  for (i = 0; i < 100; i++) {
    ...
    if (uniq && this.get(entry.addr.hostname))
      continue;

    if (i < 30 && now - entry.lastAttempt < 10 * 60)
      continue;
...
function HostList(options) {
  ...
  this.bucketsPer = 20;
  this.maxEntries = 50;
  this.maxAddresses = this.bucketsPer * this.maxEntries;
...
HostList.prototype.getHost = function getHost() {
  ...
  // Select used.
  if (this.totalUsed > 0) {
    if (this.totalFresh === 0 || util.random(0, 2))
      offset += this.bucketsPer;
  }

  for (;;) {
    index = util.random(0, this.bucketsPer);
    list = this.buckets[offset + index];

    if (list.size === 0)
      continue;

    entry = list.head;
    index = util.random(0, list.size);
...
HostList.prototype.freshBucket = function freshBucket(entry) {
  ...
  buf = new StaticWriter(size);
  buf.writeString(entry.addr.host, 'ascii');
  buf.writeString(entry.src.host, 'ascii');
  buf = buf.render();

  return murmur3(buf, 0xfba4c795) % this.bucketsPer;
};
...
HostList.prototype.usedBucket = function usedBucket(entry) {
  var buf = new Buffer(entry.addr.host, 'ascii');
  var hash = murmur3(buf, 0xfba4c795) % this.bucketsPer;
  hash += this.bucketsPer;
  return hash;
};
...
HostList.prototype.bucketSize = function bucketSize(bucket) {
  var list = this.buckets[bucket];
  assert(list, 'Bucket does not exist.');
  return list.size;
};

/**
 * Insert address into bucket.
 * @param {Number} bucket
 * @param {HostEntry} entry
 */

HostList.prototype.insert = function insert(bucket, entry) {
  var list = this.buckets[bucket];

  assert(list, 'Bucket does not exist.');
  assert(entry.bucket === -1, 'Address already belongs to a bucket.');

  if (bucket >= this.bucketsPer)
    this.totalUsed++;
  else
    this.totalFresh++;
  ...
  this.map[entry.addr.hostname] = entry;
};

/**
 * Drop address from bucket.
 * @param {HostEntry} entry
 */

HostList.prototype.drop = function drop(entry) {
  var list = this.buckets[entry.bucket];

  assert(list, 'Bucket does not exist.');

  if (entry.bucket >= this.bucketsPer)
    this.totalUsed--;
  else
    this.totalFresh--;

  entry.bucket = -1;
  assert(list.remove(entry));

  delete this.map[entry.addr.hostname];
...
HostList.prototype.add = function add(addr, src) {
  ...
  if (this.bucketSize(bucket) > this.maxEntries)
    this.evictFresh(bucket);

  this.insert(bucket, entry);
...
HostList.prototype.markAck = function markAck(hostname, services) {
  ...
  // Must be fresh.
  assert(entry.bucket < this.bucketsPer);
  ...
  if (this.bucketSize(fresh) >= this.maxEntries)
    fresh = old;

  // Swap to evicted's used bucket.
  entry.used = true;
  this.insert(evicted.bucket, entry);
  this.drop(evicted);

  // Move evicted to fresh bucket.
  evicted.used = false;
  this.insert(fresh, evicted);
};
...
HostList.prototype.evictUsed = function evictUsed(bucket) {
  ...
  for (entry = list.head; entry; entry = entry.next) {
    if (!old || old.addr.ts > entry.addr.ts)
      old = entry;
  }
