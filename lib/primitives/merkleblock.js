...
MerkleBlock.prototype.extractTree = function extractTree() {
  ...
  // This is a good validation check but only works if we know what height the merkle block will be at
  // to know what the allowed block size would be. (The check assumes the maximum number of possible
  // transactions in a block is the max block size/smallest possble bitcoin tx size == 60 bytes)
