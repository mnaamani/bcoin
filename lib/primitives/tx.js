...
TX.prototype.signatureHash = function signatureHash(index, prev, value, type, version) {
  ...
  // Traditional sighashing/bitcoin cash sighashing
  if (version === 0) {
    if(type & hashType.FORK_ID)
      return this.signatureHashBitcoinCash(index, prev, value, type)
...
TX.prototype.signatureHashBitcoinCash = function signatureHashBitcoinCash(index, prev, value, type) {
  assert(type & hashType.FORK_ID);
  return this.signatureHashV1(index, prev, value, type);
};
...
TX.prototype.signature = function signature(index, prev, value, key, type, version) {
  ...
  // sighash type should be explicitly specified
  // because we need to support both legacy bitcoin sighash and bitcoin cash sighash
  if (type == null) {
    throw new Error('sighash type not specified');
  }
