...
Wallet.prototype.sign = async function sign(mtx, passphrase) {
  ...
  if (this.db.network == 'bitcoincash' ||
      this.db.network == 'bitcoincash_testnet' ||
      this.db.network == 'bitcoincash_regtest') {
    type =  type | Script.hashType.FORK_ID
  }

  return await mtx.signAsync(rings, type, this.db.workers);
};
